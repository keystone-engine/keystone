...
bitflags! {
	pub flags Error : u32 {
		...
		const ERR_ASM_VARIANT_INVALID = 137,
		const ERR_ASM_EXPR_BRACKET = 138,
		const ERR_ASM_SYMBOL_MODIFIER = 139,
		const ERR_ASM_RPAREN = 140,
		const ERR_ASM_STAT_TOKEN = 141,
		const ERR_ASM_UNSUPPORTED = 142,
		const ERR_ASM_MACRO_TOKEN = 143,
		const ERR_ASM_MACRO_PAREN = 144,
		const ERR_ASM_MACRO_EQU = 145,
		const ERR_ASM_MACRO_ARGS = 146,
		const ERR_ASM_MACRO_LEVELS_EXCEED = 147,
		const ERR_ASM_ESC_BACKSLASH = 148,
		const ERR_ASM_ESC_OCTAL = 149,
		const ERR_ASM_ESC_SEQUENCE = 150,
