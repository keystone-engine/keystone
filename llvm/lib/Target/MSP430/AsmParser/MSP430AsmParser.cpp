...
class MSP430AsmParser : public MCTargetAsmParser {
  ...
  bool ParseOperand(OperandVector &Operands, unsigned int &ErrorCode);
...
bool MSP430AsmParser::MatchAndEmitInstruction(SMLoc Loc, unsigned &Opcode,
  ...
  case Match_Success:
    Inst.setLoc(Loc);
    Out.EmitInstruction(Inst, STI, ErrorCode);
    return false;
...
bool MSP430AsmParser::ParseInstruction(ParseInstructionInfo &Info,
  ...
  // Parse first operand
  if (ParseOperand(Operands, ErrorCode))
    return true;
...
bool MSP430AsmParser::ParseOperand(OperandVector &Operands, unsigned int &ErrorCode) {
  switch (getLexer().getKind()) {
    ...
    case AsmToken::Identifier: {
      ...
      if (!ParseRegister(RegNo, StartLoc, EndLoc, ErrorCode)) {
    ...
    case AsmToken::Minus: {
      ...
      if (!getParser().parseExpression(Val)) {
        ...
        if (getLexer().getKind() == AsmToken::LParen) {
          getLexer().Lex(); // Eat '('
          SMLoc RegStartLoc;
          if (ParseRegister(RegNo, RegStartLoc, EndLoc, ErrorCode))
            return true;
...
bool MSP430AsmParser::ParseLiteralValues(unsigned Size, SMLoc L) {
  ...
  return (getParser().parseMany(parseOne));
}

extern "C" void LLVMInitializeMSP430AsmParser() {
  RegisterMCAsmParser<MSP430AsmParser> X(llvm_ks::TheMSP430Target);
}
